...
int32_t MoveByInterpolatedStrategy(float filteredLoadReadingKg, float stepperPosFraction, ForceCurve_Interpolated* forceCurve,
                                   const DAP_calculationVariables_st* calc_st, const DAP_config_st* config_st) {
...
int32_t MoveByPidStrategy(float loadCellReadingKg, float stepperPosFraction, StepperWithLimits* stepper, ForceCurve_Interpolated* forceCurve,
                          const DAP_calculationVariables_st* calc_st, DAP_config_st* config_st, float absForceOffset_fl32, float changeVelocity) {
    if (pidWasInitialized == false) {
        ...
        myPID.SetTunings(config_st->payLoadPedalConfig_.PID_p_gain, config_st->payLoadPedalConfig_.PID_i_gain,
                         config_st->payLoadPedalConfig_.PID_d_gain);
    }
    ...
    if (control_strategy_u8 == 1) {
        float gradient_orig_fl32 = forceCurve->EvalForceGradientCubicSpline(
            config_st, calc_st, stepperPosFraction_constrained,
            true);  // determine gradient to modify the PID gain. The steeper the gradient, the less gain should be used
    ...
    // - model predictive control, see e.g.
    // https://www.researchgate.net/profile/Mohamed-Mourad-Lafifi/post/Model-Predictive-Control-examples/attachment/60202ac761fb570001029f61/AS%3A988637009301508%401612720839656/download/An+Introduction+to+Model-based+Predictive+Control+%28MPC%29.pdf
    ...
    // The setpoint comes from the force curve. The input comes from the loadcell. When the loadcell reading is below the force curve, the difference
    // becomes positive. Thus, the stepper has to move towards the foot to increase the loadcell reading. Since the QuickPID has some filtering
    // applied on the input, both variables are changed for performance reasons.
...
// #define PLOT_PID_VALUES
    ...
    static RTDebugOutput<float, 8> rtDebugFilter(
        {"stepperPosFraction", "loadCellTargetKg", "loadCellReadingKg", "loadCellReadingKg_clip", "Setpoint", "Input", "Output", "posStepperNew"});
...
int32_t MoveByForceTargetingStrategy(float loadCellReadingKg, StepperWithLimits* stepper, ForceCurve_Interpolated* forceCurve,
                                     const DAP_calculationVariables_st* calc_st, DAP_config_st* config_st, float absForceOffset_fl32,
                                     float changeVelocity, float d_phi_d_x, float d_x_hor_d_phi) {
    ...
    */
    ...
    // loadCellReadingKg_corrected += config_st->payLoadPedalConfig_.MPC_1st_order_gain * stepper_vel_filtered_fl32 / STEPS_PER_MOTOR_REVOLUTION /
    // 10;// + stepper_accel_filtered_fl32; loadCellReadingKg_corrected += config_st->payLoadPedalConfig_.MPC_1st_order_gain *
    // stepper_accel_filtered_fl32 / STEPS_PER_MOTOR_REVOLUTION / 10;// + stepper_accel_filtered_fl32;
...
#define MAX_NUMBER_OF_NEWTON_STEPS 5
    for (uint8_t iterationIdx = 0; iterationIdx < MAX_NUMBER_OF_NEWTON_STEPS; iterationIdx++) {
        ...
        // The foot is modeled to be of proportional resistance with respect to deflection. Since the deflection depends on the pedal kinematics, the
        // kinematic must be respected here This is accomplished with the forceGain variable
...
int32_t mpcBasedMove(float loadCellReadingKg, float stepperPosFraction, StepperWithLimits* stepper, ForceCurve_Interpolated* forceCurve,
                     const DAP_calculationVariables_st* calc_st, DAP_config_st* config_st, float absForceOffset_fl32) {
...
void measureStepResponse(StepperWithLimits* stepper, const DAP_calculationVariables_st* calc_st, const DAP_config_st* config_st,
                         const LoadCell_ADS1256* loadcell) {
