...
// static const uint32_t MAXIMUM_STEPPER_SPEED = (  (float)MAXIMUM_STEPPER_RPM * (float)STEPS_PER_MOTOR_REVOLUTION) / (float)SECONDS_PER_MINUTE;   //
// steps/s
...
class StepperWithLimits {
    ...
    public:
        ...
        bool hasValidStepper() const {
            return NULL != _stepper;
        }
    ...
    public:
        int32_t getLimitMin() const {
            return _limitMin;
        }
        int32_t getLimitMax() const {
            return _limitMax;
        }
        int32_t getTravelSteps() const {
            return _posMax - _posMin;
        }
