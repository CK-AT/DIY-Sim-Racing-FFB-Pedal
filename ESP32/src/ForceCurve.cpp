...
void SplineForceCurve::update(Sim *sim, float &f_sum) {
    ...
    float a = _config->cubic_spline_params_a[splineSegment_u8];
    float b = _config->cubic_spline_params_b[splineSegment_u8];

    const uint8_t *yOrig = _config->f_rel_points;
    ...
    if (_config->f_max > _config->f_min) {
        y = _config->f_min + y / 100.0f * (_config->f_max - _config->f_min);
    } else {
        y = _config->f_min;
    }
...
// float ForceCurve_Interpolated::EvalForceCubicSpline(const SplineForceCurve &config, float fractionalPos) {
...
//         splineSegment_u8 = NUMBER_OF_SPLINE_SEGMENTS - 1;
//     }
//     float a = config.cubic_spline_params_a[splineSegment_u8];
//     float b = config.cubic_spline_params_b[splineSegment_u8];

//     float yOrig[NUMBER_OF_SPLINE_SEGMENTS + 1];
//     uint8_t *yOrig = config.f_rel_points;
...
//         y = config.f_min + y / 100.0f * (config.f_max - config.f_min);
//     } else {
//         y = config.f_min;
//     }
...
// float ForceCurve_Interpolated::EvalForceGradientCubicSpline(const DAP_config_st* config_st, const DAP_calculationVariables_st* calc_st,
//                                                             float fractionalPos, bool normalized_b) {
//     float fractionalPos_lcl = constrain(fractionalPos, 0, 1);
