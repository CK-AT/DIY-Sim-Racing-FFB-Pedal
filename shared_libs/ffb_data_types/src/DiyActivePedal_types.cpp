...
const char *const axis_names[] = {"Clutch", "Brake", "Accelerator", "Pitch", "Roll", "Collective"};
...
void update_crc(uint16_t &curr_crc, const void *data, uint16_t len) {
    uint8_t sum1 = curr_crc;
    uint8_t sum2 = (curr_crc >> 8);
    int index;
    for (index = 0; index < len; index++) {
        sum1 = (sum1 + reinterpret_cast<const uint8_t *>(data)[index]) % 255;
        sum2 = (sum2 + sum1) % 255;
    }
    curr_crc = (sum2 << 8) | sum1;
}

FFBDataType verify_ffb_data_struct(const void *data, uint32_t len) {
    ...
    if (header->version != DAP_VERSION_CONFIG) return FFBDataType::NONE;
    const uint16_t *crc_loc = reinterpret_cast<const uint16_t *>(byte_ptr + len - 4);
    uint16_t crc_sent = *crc_loc;
    uint16_t crc_expected = 0;
    update_crc(crc_expected, data, len - 4);
    if (crc_expected != crc_sent) {
        /* there might be 2 bytes of padding after the CRC, try again... */
        update_crc(crc_expected, reinterpret_cast<const uint8_t *>(crc_loc), sizeof(uint16_t));
        crc_loc++;
        crc_sent = *crc_loc;
        if (crc_expected != crc_sent) return FFBDataType::NONE;
    }
    return FFBDataType(header->payloadType);
}
...
float DAP_config_st::get_f_min() const {
    return ((float)payLoadPedalConfig_.preloadForce) * 9.81f;  // to N
}

float DAP_config_st::get_f_max() const {
    return ((float)payLoadPedalConfig_.maxForce) * 9.81f;  // to N
}
