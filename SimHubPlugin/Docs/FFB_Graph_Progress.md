...
- ~~Fixed pending graph params not saved when no aircraft profile exists~~ (mechanism removed 2026-01-30: cross-session pending params file eliminated; param edits now tracked in-memory only with save prompt on vehicle change/exit).
...
- Auto-select Include context on double-click when live mode is enabled (passes Include node id into the newly opened tab and selects that context).
...
- Keyboard shortcuts: Ctrl+S Save, Ctrl+O Open (handled in GraphEditorWindow).
...
- Preview inputs/params moved to a modeless floating window with live toggle.
