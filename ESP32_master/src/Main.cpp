...
AxesManager axes_manager = AxesManager(can_manager);
...
void on_action_received(uint8_t axis_id, const DAP_actions_st *action, CommChannel comm_channel);
void on_config_received(uint8_t axis_id, const DAP_config_st *config, CommChannel comm_channel);
void on_mech_config_received(uint8_t axis_id, const DAP_mech_config_st *config, CommChannel comm_channel);
void on_bridge_state_received(const DAP_bridge_state_st *bridge_state, CommChannel comm_channel);

bool on_data_reception(const uint8_t *data, uint32_t len, CommChannel comm_channel) {
    uint8_t axis_id = reinterpret_cast<const payloadHeader *>(data)->PedalTag;
    switch (verify_ffb_data_struct(data, len)) {
        case FFBDataType::CONFIG:
            on_config_received(axis_id, reinterpret_cast<const DAP_config_st *>(data), comm_channel);
            break;
        case FFBDataType::MECH_CONFIG:
            on_mech_config_received(axis_id, reinterpret_cast<const DAP_mech_config_st *>(data), comm_channel);
            break;
        case FFBDataType::ACTION:
            on_action_received(axis_id, reinterpret_cast<const DAP_actions_st *>(data), comm_channel);
            break;
        case FFBDataType::BRIDGE_STATE:
            on_bridge_state_received(reinterpret_cast<const DAP_bridge_state_st *>(data), comm_channel);
            break;
        default:
            return false;
            break;
...
void on_bridge_state_received(const DAP_bridge_state_st *bridge_state, CommChannel comm_channel) {
    if (bridge_state->payloadBridgeState_.Bridge_action == 1) {
#ifdef ESPNow_Pairing_function
        Serial.println("[L]Bridge Pairing...");
        software_pairing_action_b = true;
#else
        Serial.println("[L]Pairing command not supported");
#endif
    }
    // action=2, restart
    if (bridge_state->payloadBridgeState_.Bridge_action == 2) {
        Serial.println("[L]Bridge Restart");
        delay(1000);
        ESP.restart();
    }
    if (bridge_state->payloadBridgeState_.Bridge_action == 3) {
...
void on_config_received(uint8_t axis_id, const DAP_config_st *config, CommChannel comm_channel) {
    axes_manager.on_config_update(axis_id, config);
    if (comm_channel == CommChannel::USB_SERIAL) {
        can_manager.send_payload_to_axis(axis_id, reinterpret_cast<const uint8_t *>(config), sizeof(DAP_config_st));
    } else {
        Serial.printf("[L]Axis %d (%s): config returned\r\n", axis_id, axis_names[axis_id]);
        Serial.write(reinterpret_cast<const uint8_t *>(config), sizeof(DAP_config_st));
        Serial.print("\r\n");
    }
}
...
void on_mech_config_received(uint8_t axis_id, const DAP_mech_config_st *config, CommChannel comm_channel) {
    if (comm_channel == CommChannel::USB_SERIAL) {
        can_manager.send_payload_to_axis(axis_id, reinterpret_cast<const uint8_t *>(config), sizeof(DAP_mech_config_st));
    } else {
        Serial.printf("[L]Axis %d (%s): mech config returned\r\n", axis_id, axis_names[axis_id]);
        Serial.write(reinterpret_cast<const uint8_t *>(config), sizeof(DAP_mech_config_st));
        Serial.print("\r\n");
    }
}
...
void on_action_received(uint8_t axis_id, const DAP_actions_st *action, CommChannel comm_channel) {
    if (comm_channel == CommChannel::USB_SERIAL) {
        if (action->payloadPedalAction_.triggerAbs_u8) {
            can_manager.send_abs_trigger_to_axis(axis_id);
        } else {
            can_manager.send_payload_to_axis(axis_id, reinterpret_cast<const uint8_t *>(action), sizeof(DAP_actions_st));
        }
        if (action->payloadPedalAction_.Rudder_action && (axis_id == 2)) {
            if (pedal_status == 0) {
                Serial.printf("[L]Pedal mode on\r\n");
                pedal_status = 1;
            } else {
                Serial.printf("[L]Pedal mode off\r\n");
                pedal_status = 0;
            }
        }
    }
...
void setup() {
    ...
    */
    ...
    //  delay(500);
    // Serial multitasking
    xTaskCreatePinnedToCore(Serial_Task, "Serial_update_Task", 5000,
                            // STACK_SIZE_FOR_TASK_2,
                            NULL, 1, &Task4, 1);
    delay(500);

#if defined(USB_JOYSTICK) && ARDUINO_USB_CDC_ON_BOOT == 0
    xTaskCreatePinnedToCore(Joystick_Task, "Joystick_update_Task", 10000,
                            // STACK_SIZE_FOR_TASK_2,
                            NULL, 1, &Task2, 0);
    delay(500);
#endif
    ...
    pixels.show();
    xTaskCreatePinnedToCore(LED_Task, "LED_update_Task", 3000,
                            // STACK_SIZE_FOR_TASK_2,
                            NULL, 1, &Task3, 0);
    delay(500);
#endif
    ...
    pixels.setPixelColor(0, 0xff, 0xff, 0xff);
    pixels.show();
    xTaskCreatePinnedToCore(LED_Task_Dongle, "LED_update_Task", 3000,
                            // STACK_SIZE_FOR_TASK_2,
                            NULL, 1, &Task3, 0);
    delay(500);
#endif
    ...
    delay(2000);
    xTaskCreatePinnedToCore(FanatecUpdate, "Fanatec_update_Task", 3000,
...
#ifdef OTA_Update
    xTaskCreatePinnedToCore(OTATask, "OTA_update_Task", 16000,
                            // STACK_SIZE_FOR_TASK_5,
                            NULL, 1, &Task7, 1);
    delay(500);
#endif
    LogOutput::printf("[L]Setup end\n");
    // initialize wifi
    for (uint i = 0; i < 30; i++) {
                ...
                if (UpdatePairingToEeprom) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (ESP_pairing_reg_local.Pair_status[i] == 1) {
                            LogOutput::printf("[L]#%iPair: %i Mac: %02X:%02X:%02X:%02X:%02X:%02X\n", i, ESP_pairing_reg_local.Pair_status[i],
                                              ESP_pairing_reg_local.Pair_mac[i][0], ESP_pairing_reg_local.Pair_mac[i][1],
                                              ESP_pairing_reg_local.Pair_mac[i][2], ESP_pairing_reg_local.Pair_mac[i][3],
                                              ESP_pairing_reg_local.Pair_mac[i][4], ESP_pairing_reg_local.Pair_mac[i][5]);
                        }
                        LogOutput::printf("\n");
                    }
                    LogOutput::printf("\n");
                    ...
                    for(int i=0; i<4;i++)
                    {
                      if(_ESP_pairing_reg.Pair_status[i]==1)
                      {
                        ...
                        if(i==1)
                        {
                          ...
                          //LogOutput::printf("[L]Mac: %02X:%02X:%02X:%02X:%02X:%02X\n", Brk_mac[0], Brk_mac[1], Brk_mac[2], Brk_mac[3], Brk_mac[4],
                    Brk_mac[5]);
...
void update_controller_output(void) {
    int32_t clutch_value = NormalizeControllerOutputValue(axes_manager.get_controller_value(0), 0.0, 1.0, 100.0);
    int32_t brake_value = NormalizeControllerOutputValue(axes_manager.get_controller_value(1), 0.0, 1.0, 100.0);
    int32_t throttle_value = NormalizeControllerOutputValue(axes_manager.get_controller_value(2), 0.0, 1.0, 100.0);
    if (IsControllerReady()) {
        if (pedal_status == 0) {
            SetControllerOutputValueClutch(clutch_value);
            SetControllerOutputValueBrake(brake_value);
            SetControllerOutputValueThrottle(throttle_value);
            SetControllerOutputValueRudder(0);
            SetControllerOutputValueRudder_brake(0, 0);
        }
        if (pedal_status == 1) {
            SetControllerOutputValueClutch(0);
            SetControllerOutputValueBrake(0);
            SetControllerOutputValueThrottle(0);
            SetControllerOutputValueRudder(throttle_value);
            SetControllerOutputValueRudder_brake(0, 0);
        }
        joystickSendState();
    }
...
#ifdef OTA_Update
                // not an FFB data type, might be Basic_WIfi_info
                if (n == sizeof(Basic_WIfi_info)) {
                    Serial.printf("[L]get basic wifi info\r\n");
                    memcpy((char *)&_basic_wifi_info, buffer, sizeof(Basic_WIfi_info));
...
#endif
                {
                    Serial.printf("[L]%d bytes of unknown data reveived\r\n", n);
                }
            }
        }

        if ((current_time - bridge_state_last_update) > 100) {
            bridge_state_last_update = millis();
            int rssi_filter_value = -10;
            ...
            DAP_state_basic_st basic_state;
            for (uint8_t axis_id = 0; axis_id < 3; axis_id++) {
                if (can_manager.is_online(axis_id)) {
                    if (dap_bridge_state_st.payloadBridgeState_.Pedal_availability[axis_id] == 0) {
                        Serial.printf("[L]Axis %d (%s): connected\r\n", axis_id, axis_names[axis_id]);
                    }
                    dap_bridge_state_st.payloadBridgeState_.Pedal_availability[axis_id] = 1;
                } else {
                    if (dap_bridge_state_st.payloadBridgeState_.Pedal_availability[axis_id] == 1) {
                        Serial.printf("[L]Axis %d (%s); disconnected\r\n", axis_id, axis_names[axis_id]);
                    }
                    dap_bridge_state_st.payloadBridgeState_.Pedal_availability[axis_id] = 0;
                }
                axes_manager.populate_basic_state(axis_id, basic_state);
                Serial.write((char *)&basic_state, sizeof(DAP_state_basic_st));
                Serial.print("\r\n");
            }
            dap_bridge_state_st.payloadFooter_.checkSum = 0;
            update_crc(dap_bridge_state_st.payloadFooter_.checkSum, &dap_bridge_state_st,
                       sizeof(DAP_bridge_state_st::payLoadHeader_) + sizeof(DAP_bridge_state_st::payloadBridgeState_));
            Serial.write((char *)&dap_bridge_state_st, sizeof(DAP_bridge_state_st));
            Serial.print("\r\n");
        }

#if defined(USB_JOYSTICK) && ARDUINO_USB_CDC_ON_BOOT == 1
        delay(2);
...
void LED_Task(void *pvParameters) {
    for (;;) {
        ...
        if (LED_Status == 0) {
            ...
            uint8_t led_status = dap_bridge_state_st.payloadBridgeState_.Pedal_availability[0] +
                                 dap_bridge_state_st.payloadBridgeState_.Pedal_availability[1] * 2 +
                                 dap_bridge_state_st.payloadBridgeState_.Pedal_availability[2] * 4;
            switch (led_status) {
                case 0:
                    pixels.setPixelColor(0, 0xff, 0xff, 0xff);
